...
async function loadStatus() {
  ...
  headers: {
    ...
    setMessage("❌ Failed to load parking status");
  }
}
  ...
  async function carEntry() {
    if (!validateVehicleNumber(carNumber)) {
        setMessage("❌ Invalid vehicle number. Format: AA 00 BB 0000");
        return;
    }
    ...
    try {
      ...
      setMessage(`✅ Parked at ${data.slot}`);
      setHighlightSlot(data.slot);
      loadStatus();
    } catch (e) {
      setMessage(e?.detail ?? "❌ Server error");
    }
  }
  ...
  async function carExit() {
    ...
    try {
      ...
      setMessage(`🚪 Exited from ${data.slot}`);
  ...
  try {
    ...
    setHighlightSlot(data.slot);
    setMessage(`✅ Vehicle parked at ${data.slot}`);
  } catch (e) {
    setHighlightSlot(null);
