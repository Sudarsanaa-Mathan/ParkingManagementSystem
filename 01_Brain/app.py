...
def build_grid_from_db(db: Session):
    ...
    for slot in occupied_slots:
        try:
            ...
            #else:
            #    print(f"⚠️ Ignoring invalid slot: {slot}")

        #except Exception as e:
        #    print(f"❌ Slot parse failed: {slot}, error: {e}")
        except Exception:
            pass
...
@app.get("/parking/status")
def parking_status(user=Depends(get_current_user)):
    db = SessionLocal()
    try:
...
@app.post("/car/entry")
def car_entry(req: EntryRequest, user=Depends(get_current_user)):
    ...
    #    raise
    #except Exception as e:
    #    print("🔥 ERROR in /car/entry:", repr(e))
...
# ---------------------------------------------------------
# CAR EXIT (DB ONLY — NO RAM LOGIC)
# ---------------------------------------------------------

@app.post("/car/exit")
def car_exit(req: ExitRequest, user=Depends(get_current_user)):
    db: Session = SessionLocal()
    try:
        car_number = validate_vehicle_number(req.car_number)
...
@app.get("/admin/parking/config")
def admin_config(user=Depends(require_role("ADMIN"))):
    return {"message": "Admin access granted"}

@app.get("/security/grid")
def security_grid(user=Depends(require_role("SECURITY"))):
    return {"message": "Security access granted"}
